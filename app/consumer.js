...
class App {
  ...
  async _connectToMQ() {
    ...
    logger.info(`Professor-Diagnoses-Error : connected to rabbitmq at ${Nconf.get('AMQP_URI')}`);
  ...
  async _closeMQConnection() {
    await MQ.close();
    logger.info(`Professor-Diagnoses-Error : disconnected from rabbitmq at ${Nconf.get('AMQP_URI')}`);
  }

  // eslint-disable-next-line class-methods-use-this
  _connectToRedis() {
    Redis.connect(Nconf.get('REDIS_URI'), Nconf.get('REDIS_INDEX'));
    logger.info(`Professor-Diagnoses-Error : connected to redis at ${Nconf.get('REDIS_URI')}/${Nconf.get('REDIS_INDEX')}`);
  }

  // eslint-disable-next-line class-methods-use-this
  async _closeRedisConnection() {
    await Redis.close();
    logger.info(`Professor-Diagnoses-Error : disconnected from redis at ${Nconf.get('REDIS_URI')}/${Nconf.get('REDIS_INDEX')}`);
  }
  ...
  async stop() {
    ...
    logger.info(`Professor-Diagnoses-Error : shutting down`);
  }
}
