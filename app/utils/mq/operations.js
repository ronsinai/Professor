...
class MQOperations {
  ...
  async _msgHandler(msg) {
    ...
    try {
      ...
      if (!diagnoses.length) {
        await this.publish(diagnosis, imagingId);
      ...
      return logger.error(`Rejected ${diagnosis} diagnosis error of imaging ${imagingId} with requeue=${this.REQUEUE_ON_PUB_ERR}`);
    }
