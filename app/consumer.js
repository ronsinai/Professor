...
class App {
  ...
  async _connectToMQ() {
    ...
    logger.info(`Professor-Discharges : connected to rabbitmq at ${Nconf.get('AMQP_URI')}`);
  ...
  async _closeMQConnection() {
    await MQ.close();
    logger.info(`Professor-Discharges : disconnected from rabbitmq at ${Nconf.get('AMQP_URI')}`);
  }

  // eslint-disable-next-line class-methods-use-this
  _connectToRedis() {
    Redis.connect(Nconf.get('REDIS_URI'), Nconf.get('REDIS_INDEX'));
    logger.info(`Professor-Discharges : connected to redis at ${Nconf.get('REDIS_URI')}/${Nconf.get('REDIS_INDEX')}`);
  }

  // eslint-disable-next-line class-methods-use-this
  async _closeRedisConnection() {
    await Redis.close();
    logger.info(`Professor-Discharges : disconnected from redis at ${Nconf.get('REDIS_URI')}/${Nconf.get('REDIS_INDEX')}`);
  }
  ...
  async stop() {
    ...
    logger.info(`Professor-Discharges : shutting down`);
  }
}
